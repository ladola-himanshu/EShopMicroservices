...
namespace CatalogAPI.Data
{
    public class CatalogInitialData : IInitialData
    {
            ...
            CancellationToken cancellation)
        {
            ...
            if (await session.Query<dto.Product>().AnyAsync())
                return;
            session.Store<dto.Product>(GetPreconfiguredData());
            await session.SaveChangesAsync();
        }

        private static IEnumerable<dto.Product> GetPreconfiguredData() => new List<dto.Product>()
        {
            new dto.Product()
            {
